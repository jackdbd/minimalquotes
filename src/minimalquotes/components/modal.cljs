...
(defn- validate-modal-window!
  ...
  (let [n (.-length (.querySelectorAll js/document (str "#" modal-window-id)))]
    (when (not= 1 n)
      (throw (js/Error. (str "There are " n " components with id = " modal-window-id " on this page. There should be only 1."))))))

(defn modal!
  "Dispatch a custom event to open/close a modal."
  [hiccup-content]
  (let [elem (.querySelector js/document (str "#" modal-window-id))
        event (js/CustomEvent. modal-event-type #js {:detail {:child hiccup-content}})]
...
(defn modal-window
  ...
  [{:keys [modal-backdrop-css-class
           modal-window-css-class
           modal-window-hidden-css-class
           on-after-close
           on-after-open
           should-close-on-esc]
  ...
  (let [child (r/atom nil)
        close-modal (fn []
                      (reset! child nil)
                      (on-after-close))
        open-modal (fn [hiccup-content]
                     (reset! child hiccup-content)
                     (on-after-open))
        on-keydown (fn [e]
        ...
        reagent-render (fn []
                         [:div {:id modal-window-id
                          ...
                          (when @child
                            [:div {:class [modal-backdrop-css-class]
                                   :data-backdrop "true"
