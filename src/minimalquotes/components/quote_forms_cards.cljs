...
(defn- on-click-cancel
  []
  (js/alert "Cancel"))

(defn- on-submitted-values
  [m]
...
(defcard quote-form-modal-buttons-card
  ...
  (let [props {:on-submitted-values on-submitted-values
               :tags fakes/tags}]
    (dc/reagent [:div
                 [modal-window]
                 [button-add-new-quote-modal props]
                 [button-edit-quote-modal (merge props {:quote-author fakes/author-0
                                                        :quote-text fakes/text-0})]
                 [button-delete-quote-modal {:on-delete (fn [author]
