...
(defcard
  ...
  (let [props {:entries fakes/quotes
               :on-delete-quote (fn [user-id quote-id]
                                  (js/alert (str "user " user-id
                                              " deletes quote " quote-id)))
               :on-edit-quote (fn [user-id quote-id]
                                (js/alert (str "user " user-id
                                            " edits quote " quote-id)))
               :on-like-quote (fn [user-id quote-id]
                                (js/alert (str "user " user-id
                                            " likes quote " quote-id)))
               :on-share-quote (fn [user-id quote-id]
                                 (js/alert (str "user " user-id
                                             " shares quote " quote-id)))}]
...
(defn quotes-example
  ...
  (let [props {:entries fakes/quotes
               :delete-quote! (fn [quote-id]
                                (js/alert (str "delete quote " quote-id)))
